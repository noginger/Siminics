...
namespace BaseLibrary.MySqlConfig
{
    public abstract class DbBase<TEntity> where TEntity : new()
    {
        ...
        public virtual TEntity GetItem(int identity)
        {
            ...
            entity = MySqlHelper.ExecuteObject<TEntity>(string.Format("select * from {0} where {1} = {2}", TableName, ColumnKey, identity));
        ...
        public virtual TEntity GetItem(string keyValue)
        {
            ...
            entity = MySqlHelper.ExecuteObject<TEntity>(string.Format("select * from {0} where {1} = '{2}'", TableName, ColumnKey, MySqlHelper.FilterSpecialChar(keyValue)));
        ...
        public List<TEntity> GetItems(DbQueryParamtersNoPage paramters)
        {
            ...
            //执行
            return MySqlHelper.ExecuteObjects<TEntity>(sb.ToString());
        }
        ...
        public ListBag<int, List<TEntity>> GetItems(DbQueryParamters paramters)
        {
            ...
            if (paramters.IsCount)
            {
                if (paramters.IsSameColumnKey)
                {
                    ...
                    if (!string.IsNullOrEmpty(paramters.Join) && paramters.Join.IndexOf("On") > -1)
                        sbCount.Append(string.Format("SELECT COUNT({0}) AS RecordCount FROM {1} USING({0}) {2}", ColumnKey, paramters.TableName.Substring(0, paramters.TableName.IndexOf("On")), string.IsNullOrEmpty(paramters.Condition) ? string.Empty : "Where " + paramters.Condition));
                    else if (string.IsNullOrEmpty(paramters.Condition))
                        sbCount.Append(string.Format("SELECT COUNT({0}) AS RecordCount FROM {1} USING({0})", ColumnKey, paramters.TableName));
                    else
                        sbCount.Append(string.Format("SELECT COUNT({0}) AS RecordCount FROM {1} USING({0}) where {2}", ColumnKey, paramters.TableName, paramters.Condition));
                    //分组条数  修改目的：有Groupby时的总条数
                    if (!string.IsNullOrEmpty(paramters.GroupBy))
                        sbCount.AppendFormat(paramters.GroupBy);
                    sbCount.Append(";");
                }
                else
                {
                    ...
                    {
                        if (string.IsNullOrEmpty(paramters.Condition))
                            sbCount.Append(string.Format("SELECT COUNT({0}) AS RecordCount FROM {1}", ColumnKey, paramters.JoinTable));
                        else
                            sbCount.Append(string.Format("SELECT COUNT({0}) AS RecordCount FROM {1} where {2}", ColumnKey, paramters.JoinTable,
                                                    paramters.Condition));
                    }
                    else
                    {
                        if (string.IsNullOrEmpty(paramters.Condition))
                            sbCount.Append(string.Format("SELECT COUNT({0}) AS RecordCount FROM {1}", ColumnKey, paramters.TableName));
                        else
                            sbCount.Append(string.Format("SELECT COUNT({0}) AS RecordCount FROM {1} where {2}", ColumnKey, paramters.TableName,
                                                    paramters.Condition));
                    }
                    //分组条数 修改目的：有Groupby时的总条数
                    if (!string.IsNullOrEmpty(paramters.GroupBy))
                        sbCount.AppendFormat(paramters.GroupBy);
                    //sbCount.AppendFormat(";");
                }
            ...
            lists = MySqlHelper.ExecuteObjects<TEntity>(sb.ToString());
            recordCount = new MySqlHelper().ExecuteScalar<int>(sbCount.ToString());
